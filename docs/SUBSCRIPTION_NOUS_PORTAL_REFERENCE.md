...
ForgeRouter reference for Nous Portal as an OAuth subscription provider —
the inference backend behind Hermes Agent (Nous Research).
...
- The Hermes Agent project's own documentation for this exact scenario
  ...
  that injects a live Nous bearer into every request. External apps point
  at it with *any* placeholder token.
