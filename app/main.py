...
def admin_agent_create(payload: AgentPayload, request: Request):
    ...
    api_key = payload.api_key.strip() or f"hermes_{secrets.token_urlsafe(30)}"
    try:
        create_agent(name, api_key, payload.description.strip())
    except Exception as exc:
        duplicate = "unique" in str(exc).lower() or "duplicate" in str(exc).lower()
        return JSONResponse(
            status_code=409 if duplicate else 500,
...
def admin_agent_rotate_key(name: str, request: Request):
    ...
    if auth_error:
        return auth_error
    api_key = f"hermes_{secrets.token_urlsafe(30)}"
    try:
